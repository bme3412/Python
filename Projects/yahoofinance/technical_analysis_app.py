...
# def a function to download a list of S&P 500 constituents:
@st.cache
...
# define a function for downlaoding historical stock prices with yfinance

@st.cache
def load_data(symbol, start, end):
    return yf.download(symbol, start, end)

# define a fucntion for stoing downloaded data as a CSV file
...
# define the par tof the sidebar used for tuning the details of the technical analysis
...
# add the expander with the parameters of the Bollinger Band
...
bb_std = exp_bb.number_input(label='# of std deviations', min_value=1, max_value=4, value=2, step=1)

# add the expadner with parameters of RSI
...
# specify title
st.title('A web app for technical analysis')
st.write("""
### User manual
# * you can select any of the companies""")

# load historical prices
df = load_data(ticker, start_date, end_date)

# add the expander with a preview
...
if rsi_flag:
    ...
    rsi_upper=rsi_upper, rsi_lower=rsi_lower,showbands=True)
